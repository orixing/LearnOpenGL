...
void main() {
    ...
    {   
        ...
        if(abs(lumaEndN1 - lumaM) > 0.15 || abs(lumaEndN2 - lumaM) > 0.15 || abs(lumaEndN1 - lumaOther) > 0.15 || abs(lumaEndN2 - lumaOther) > 0.15){
            FragColor = vec4(rgbM,1.0);
            return;
        }
        //如果两个像素差距较小，认为边界到头了
    ...
    {   
        ...
        //如果出现和原始像素对差异较大的颜色，则这段可能不是锯齿
        if(abs(lumaEndP1 - lumaM) > 0.15 || abs(lumaEndP2 - lumaM) > 0.15 || abs(lumaEndP1 - lumaOther) > 0.15 || abs(lumaEndP2 - lumaOther) > 0.15){
            FragColor = vec4(rgbM,1.0);
            return;
        }
        //如果两个像素差距较小，认为边界到头了
